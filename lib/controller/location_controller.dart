...
class LocationController extends GetxController implements GetxService {
  ...
  Future<List<SearchModel>> searchProducts(
      BuildContext context, String text) async {
    List<SearchModel> _searchList = [];
    if (text != null && text.isNotEmpty) {
      Response response = await locationRepo.searchProducts(text);
      if (response.statusCode == 200) {
        response.body.forEach(
            (prediction) => _searchList.add(SearchModel.fromJson(prediction)));
      } else {
        showCustomSnackBar(
            response.body['error_message'] ?? response.bodyString);
      }
